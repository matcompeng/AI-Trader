...
class Predictor:
    ...
    def format_stable_prompt(self, all_features, current_price, historical_data_1,historical_data_2, current_obv):
        ...
        prompt = (
           ...
           f"       - *StochRSI Check*: Is the current %K (from the current market data interval '{self.trading_interval}') crossing above the current %D, while %K has recently transitioned from below 20 within the last 4 records of the '{self.trading_interval}' historical context data? If both conditions are true, proceed to check 'OBV Confirmation'.\n"
