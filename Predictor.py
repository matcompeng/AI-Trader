...
class Predictor:
    ...
    def format_stable_prompt(self, all_features, current_price, historical_data_1,historical_data_2, current_obv):
        ...
        prompt = (
            ...
            "2. Buy After Dip Reversal: Consider 'Buy' decisions when the price shows a strong reversal from a dip. Ensure that the price demonstrates upward momentum in key indicators, such as RSI is transitioning from oversold, OBV is increasing, and MACD Histogram shows consecutive positive bars or multiple tests of a support level. This point should primarily be used during high volatility or sudden market dips.\n"
            "3. Buy After Resistance Breakout: Consider 'Buy' decisions only if the price closes above the latest significant resistance level and aligns with key technical indicators (e.g., positive MACD Histogram, RSI < 60 and ADX > 20) identified from the Historical Contexts, confirming continued upward momentum. Ensure that the price has established itself above the resistance level, rather than hovering near it. Use this point during periods of stable trending market conditions.\n"
            "4. Decision Priority: If both Point 1 (support reversal) and Point 2 (resistance breakout) appear valid:\n"
            ...
            "6. Avoid Overbought Conditions: if Point 2 (resistance breakout) appear valid ,Avoid 'Buy' decisions if the price enters an overbought condition (RSI > 70) in any of the shorter intervals ('1m', '5m', '15m') in current market data, as these signals indicate potential for a pullback in these timeframes.\n"
            "7. Overall Market Momentum: Ensure that the overall market momentum supports the 'Buy' decision by looking for upward movement across key indicators, particularly when these indicators transition from neutral or negative zones to positive trends.\n"
        )
